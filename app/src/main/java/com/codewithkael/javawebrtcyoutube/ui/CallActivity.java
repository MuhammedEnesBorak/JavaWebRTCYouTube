...
public class CallActivity extends AppCompatActivity implements MainRepository.Listener {
    ...
    private void init(){
        ...
        views.callBtn.setOnClickListener(v->{
            String targetUserName = views.targetUserNameEt.getText().toString();
            String roomName = "yourRoomName"; // Oda ismi burada sabitlenebilir ya da dinamik olarak alınabilir

            if (mainRepository.canJoinRoom(roomName, targetUserName)) {
                //start a call request here
                mainRepository.sendCallRequest(targetUserName, ()->{
                    Toast.makeText(this, "Could not find the target", Toast.LENGTH_SHORT).show();
                });
            } else {
                Toast.makeText(this, "Room is full. Max 20 participants allowed.", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        views.endCallButton.setOnClickListener(v->{
            mainRepository.leaveRoom("yourRoomName", "currentUserName");
